...
**Thay thế/sửa đổi:** [2026-08-23-deep-crawl-threads-design.md](./2026-08-23-deep-crawl-threads-design.md) (§4 chọn topic, §7 ngân sách), sub-project 2c (Facebook seed-page crawl — giữ nguyên crawl, đổi vai trò), sub-project 3 phần 1 (sentiment classification — loại bỏ).
...
4. **Facebook giữ nguyên cơ chế crawl** (6 trang seed cố định, không đổi actor/luồng gọi) — chỉ THÊM bước trích từ khóa từ nội dung bài, y hệt cơ chế Threads mới.
...
flowchart TD
    ...
    FB["Facebook: crawl 6 trang seed\n+ trích từ khóa từ bài viết"] --> CT
...
## 5. Facebook — thêm bước trích từ khóa

`deep-crawl-facebook.ts` giữ nguyên toàn bộ phần crawl (6 trang seed, actor, cách gọi). Chỉ thêm, ngay sau khi nhận được bài viết thô của 1 trang:

1. `extractKeywords(post.text_content)` (cùng field `FacebookPost.text_content` đang dùng cho sentiment/lưu trữ hiện tại) trên từng bài.
2. Cộng dồn engagement (`FacebookPost`: `like_count + comment_count + share_count`, coi null là 0) theo từ khóa trích được, `category` = category có sẵn của trang (`FacebookSeedPage.category`) — không cần đoán category, đã biết chắc từ seed list.
3. Ghi `candidate_topics` (`source: 'facebook'`).
4. Bỏ lời gọi sentiment (vốn đã tách job riêng `run-classify-sentiment.ts` — job này bị xoá hoàn toàn, không cần sửa gì trong `deep-crawl-facebook.ts` để "bỏ" sentiment vì nó chưa từng gọi trực tiếp ở đây).
...
| Facebook | 6 trang/ngày, ~$0.005-0.026/bài, biến động tới ~$70/tháng worst-case | Không đổi (crawl y hệt) |

Tổng ngân sách gần như giữ nguyên — đổi cách phân bổ (từ "8 điểm cụ thể" sang "6 lát cắt rộng theo category"), không xin thêm ngân sách.
...
- Đổi/tinh chỉnh danh sách query Threads hoặc trang seed Facebook sau khi có dữ liệu thật (điều chỉnh không cần đổi kiến trúc, xem §4/§5).
...
- Mới: `src/lib/threads-discovery-queries.ts` (hằng số §4), `src/lib/aggregate-threads-keywords.ts` (logic trích + cộng dồn, tương tự `aggregate-youtube-keywords.ts`), `src/lib/aggregate-facebook-keywords.ts` (tương tự, cho Facebook).
- Sửa: `src/deep-crawl.ts` (bỏ chọn-8-topic, lặp qua `THREADS_DISCOVERY_QUERIES`), `src/deep-crawl-facebook.ts` (thêm bước trích từ khóa, bỏ nhánh gọi sentiment nếu có), `src/types.ts` + `dashboard/lib/types.ts` (mở rộng `CandidateTopic['source']`), mọi `Record<CandidateTopic['source'], ...>`/switch exhaustive trong dashboard (`hot-topics.ts`'s `groupBySource`, `hot-topic-format.ts`'s `SOURCE_LABELS`, `trending.ts`'s `flattenAndRankHotTopics` param type, v.v. — plan liệt kê đủ).
