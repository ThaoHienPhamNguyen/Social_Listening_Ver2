...
**Đã fix (2026-08-23, phát hiện từ dashboard production — cột "Topic đang hot" toàn từ khoá vô nghĩa/1 từ):**

- **Lỗi decode HTML entity trong tiêu đề RSS**: feed của Thanh Niên (và có thể publisher khác) encode dấu tiếng Việt bằng named HTML entity (`&ecirc;`, `&ocirc;`, `&aacute;`...) thay vì UTF-8 thô. Đây không phải entity XML chuẩn nên `rss-parser`/`xml2js` không giải mã được, để nguyên text `&ecirc;` lọt vào `articles.title`/`snippet`; `extractKeywords()` sau đó strip `&`/`;` nhưng giữ lại chữ cái giữa, sinh token rác kiểu "ecirc"/"igrave". Sửa bằng `decodeHtmlEntities()` (`src/lib/decode-html-entities.ts`, wrap thư viện `entities`'s `decodeHTML` — decode một chiều, không re-encode như `sanitizeHtml`/`htmlToText` nên không tạo bug mới cho tiêu đề có `&` thật) — gọi ở `ingest-rss.ts` trên cả `title` và `snippet` trước khi lưu và trước khi đưa vào `categorize()`. Áp dụng cho lần crawl mới trở đi, không backfill dòng cũ (theo đúng tiền lệ đã chọn cho `full_content`/`sanitize-html`, xem RSS ingestion schema doc).
- **`extractKeywords()` không còn sinh từ đơn (unigram)** — trước đây mỗi văn bản sinh cả từ đơn lẫn cụm đôi, cạnh tranh ngang hàng trong shortlist theo tần suất/view-count thô. STOP_WORDS (27 từ hư từ tiếng Việt) không phủ được đại từ/từ chung chung khác (mình, tôi, bạn, hoàng, văn, khu...) hay từ lóng/tiếng Anh lẫn trong tiêu đề (lol, gen...) — các từ đơn phổ biến này tích luỹ count lớn và đẩy văng cụm từ có nghĩa khỏi top-10/nguồn. Quyết định: bỏ hẳn unigram, chỉ giữ cụm 2 từ (đổi từ "giữ cụm 1-2 từ" ở spec §5 gốc). Đánh đổi: mất khả năng phát hiện từ khoá đơn thật sự có nghĩa đứng riêng lẻ (vd "bitcoin" nếu không đi kèm từ láng giềng ổn định) — chấp nhận được vì tên riêng/thương hiệu thường vẫn xuất hiện trong cụm đôi ổn định (vd "giá bitcoin").
- **Tag YouTube (`snippet.tags`) KHÔNG áp dụng thay đổi trên** — vẫn giữ nguyên hành vi cũ (kể cả tag đơn từ), quyết định có chủ đích: tag là do creator tự đặt (tín hiệu chủ đích, gần giống Google Trends hơn là câu văn tự nhiên) chứ không phải câu tự nhiên nhiều từ rác — không có rule tự động nào (độ dài, stop-word tiếng Việt) phân biệt được tag đơn từ có nghĩa (vd "bitcoin", tên riêng/idol) khỏi tag rác tiếng Anh (vd "gen", "troll", "top", "tour") mà không cần allowlist/blocklist tay. Vẫn còn lẫn một phần rác tiếng Anh/lóng từ tags — chưa xử lý, cần allowlist/blocklist tay nếu muốn dọn tiếp.
...
**Đã fix (2026-08-23, phát hiện qua dashboard production — 1 nguồn hiển thị ~39 dòng thay vì tối đa ~10-40 dự kiến):**

- **`is_shortlisted` giờ được reset về `false` cho toàn bộ `date` hôm nay ở đầu mỗi lần `rank-and-select` chạy** (`CandidateTopicRepository.resetShortlisted(date)`, gọi ngay sau `getTodayCandidates()`), trước khi tính lại top-N. Trước fix: cờ chỉ từng được set `true` (`markShortlisted`), không bao giờ bị xoá — nên qua 3 lần cron chạy/ngày, một từ khoá chỉ cần lọt top-N ở BẤT KỲ lần chạy nào trong ngày là giữ cờ `true` cho tới hết ngày, cộng dồn vượt xa top-10/nguồn dự kiến (dashboard không tự giới hạn lại khi hiển thị, chỉ lọc `is_shortlisted=true`). Giờ mỗi lần chạy phản ánh đúng bức tranh hiện tại, không cộng dồn nữa — cùng triết lý "tính lại mỗi lần chạy" đã áp dụng cho `growth_rate`.
- Lưu ý: sub-project 2b (Apify deep-crawl) vẫn nên tự lọc theo `date` khi đọc `is_shortlisted`, không giả định nó luôn phản ánh đúng lần chạy gần nhất trong ngày nếu đọc giữa 2 lần cron.
