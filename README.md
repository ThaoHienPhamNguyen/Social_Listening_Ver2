...
- Migration `0005_add_facebook_page_data.sql` is applied to production and the `deep-crawl-facebook` job is live-verified via a real `workflow_dispatch` run (`pagesAttempted=6 postsUpserted=52 errors=0`, all 6 pages returned data — none hit `no_items`/`not_available`). See `docs/superpowers/specs/2026-08-23-deep-crawl-facebook-database-schema.md` for the schema.
